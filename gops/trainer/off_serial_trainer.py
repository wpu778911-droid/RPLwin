...
class OffSerialTrainer:
    def __init__(self, alg, sampler, buffer, evaluator, **kwargs):
        ...
        self.iteration = 0
        self.debug_first_iters = kwargs.get("debug_first_iters", 0)
        self.print_each_iter = kwargs.get("print_each_iter", False)
        ...
        # pre sampling
        warm_size = kwargs["buffer_warm_size"]
        last_warm_log = time.time()
        while self.buffer.size < warm_size:
            samples, _ = self.sampler.sample()
            self.buffer.add_batch(samples)
            now = time.time()
            if now - last_warm_log >= 2.0:
                print(
                    "Warm-up sampling... buffer size {}/{}".format(
                        self.buffer.size, warm_size
                    )
                )
                last_warm_log = now
    ...
    def step(self):
        debug = self.iteration < self.debug_first_iters
        # sampling
        if self.iteration % self.sample_interval == 0:
            if debug:
                print("[debug] iter {} sampling start".format(self.iteration), flush=True)
            ...
            if debug:
                print("[debug] iter {} sampling done".format(self.iteration), flush=True)

        # replay
        if debug:
            print("[debug] iter {} replay sample start".format(self.iteration), flush=True)
        replay_samples = self.buffer.sample_batch(self.replay_batch_size)
        if debug:
            print("[debug] iter {} replay sample done".format(self.iteration), flush=True)
        ...
        if self.per_flag:
            if debug:
                print("[debug] iter {} alg update start".format(self.iteration), flush=True)
        ...
        self.networks.eval()
        if debug:
            print("[debug] iter {} alg update done".format(self.iteration), flush=True)

        # log
        if self.print_each_iter or self.iteration % self.log_save_interval == 0:
