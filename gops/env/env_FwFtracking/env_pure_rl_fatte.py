...
class ToRwheelsimRobot(Robot):
    def __init__(
        ...
        steer_flip_threshold: float = np.pi / 2.0,
    ):
        self.dt = dt
        ...
        self._steer_prev_step = {name: 0.0 for name in WHEEL_ORDER}
        self._vel_world = np.zeros(2, dtype=np.float32)
        self._w_body = 0.0
        self._steer_flip_threshold = float(steer_flip_threshold)
    ...
    def step(self, action: np.ndarray) -> np.ndarray:
        ...
        self._steer_prev_step = dict(self._steer)
        ...
        for i, name in enumerate(WHEEL_ORDER):
            ...
            # Prefer the equivalent steer (theta or theta+pi) closer to previous steer
            steer_alt = wrap_to_pi(steer + np.pi)
            if abs(wrap_to_pi(steer - self._prev_steer[name])) > self._steer_flip_threshold:
                if abs(wrap_to_pi(steer_alt - self._prev_steer[name])) < abs(
                    wrap_to_pi(steer - self._prev_steer[name])
                ):
                    steer = steer_alt
                    speed = -speed
...
class ToRwheelsimEnv(Env):
    ...
    def __init__(
        ...
        *,
        ...
        w_slip_end: float = 0.1,
        w_slip_warm_steps: int = 150000,
        w_ik_speed_start: float = 0.0,
        w_ik_speed_end: float = 0.1,
        w_ik_steer_start: float = 0.0,
        w_ik_steer_end: float = 0.1,
        w_ik_warm_steps: int = 80000,
        w_steer_cont_start: float = 0.0,
        w_steer_cont_end: float = 0.2,
        w_steer_flip: float = 0.1,
        w_steer_limit: float = 0.05,
        steer_flip_threshold: float = np.pi / 2.0,
        steer_limit_margin: float = np.deg2rad(5.0),
        w_steer_smooth_start: float = 0.02,
        w_steer_smooth_end: float = 0.08,
        w_steer_smooth_warm_steps: int = 80000,
        **kwargs,
    ):
        self.robot = ToRwheelsimRobot(
            ...
            steer_flip_threshold=steer_flip_threshold,
        )
        ...
        self._w_ik_speed_start = float(w_ik_speed_start)
        self._w_ik_speed_end = float(w_ik_speed_end)
        self._w_ik_steer_start = float(w_ik_steer_start)
        self._w_ik_steer_end = float(w_ik_steer_end)
        self._w_ik_warm_steps = int(w_ik_warm_steps)
        self._w_steer_cont_start = float(w_steer_cont_start)
        self._w_steer_cont_end = float(w_steer_cont_end)
        self._w_steer_flip = float(w_steer_flip)
        self._w_steer_limit = float(w_steer_limit)
        self._steer_flip_threshold = float(steer_flip_threshold)
        self._steer_limit_margin = float(steer_limit_margin)
        self._w_steer_smooth_start = float(w_steer_smooth_start)
        self._w_steer_smooth_end = float(w_steer_smooth_end)
        self._w_steer_smooth_warm_steps = int(w_steer_smooth_warm_steps)
        self.seed()
    ...
    def _get_reward(self, action: np.ndarray) -> float:
        ...
        bx, by, bdx, bdy = self.context.state.reference
        rx, ry, yaw_ref = self._get_ref_target()
        # Main tracking terms: position/velocity/yaw errors to the reference.
        pos_err = float(np.hypot(px - rx, py - ry) / max(self.follow_dist, 1e-6))
        vel_err = float(np.hypot(vx - bdx, vy - bdy) / max(BALL_SPEED, 1e-6))
        yaw_err_raw = abs(wrap_to_pi(yaw - yaw_ref)) / np.pi
        yaw_tol = np.deg2rad(1.5) / np.pi
        yaw_err = max(0.0, yaw_err_raw - yaw_tol)
        self._yaw_err_ema = (
            self._yaw_err_ema_alpha * self._yaw_err_ema
            + (1.0 - self._yaw_err_ema_alpha) * yaw_err
        )
        action = clip(action, self.action_space.low, self.action_space.high)
        ctrl_cost = 0.0
        for i in range(4):
        ...
        # Wheel coordination penalties: discourage opposing wheel directions and wasted speed.
        dir_penalty = float(np.sum(np.linalg.norm(wheel_vecs - v_mean, axis=1) ** 2))
        waste_penalty = float(np.sum(np.linalg.norm(wheel_vecs, axis=1) ** 2) - 4.0 * np.linalg.norm(v_mean) ** 2)

        # Wheel efficiency: align wheel velocity with desired chassis twist (v_ref + omega_ref x r).
        # omega_ref comes from the reference yaw rate (not from a control law).
        omega_des = float(np.clip(self._yaw_ref_rate, -0.5 * self.w_max, 0.5 * self.w_max))
        cos_yaw = np.cos(yaw)
        sin_yaw = np.sin(yaw)
        v_des_world = np.array([bdx, bdy], dtype=np.float32)
        eff_scores = []
        for name in WHEEL_ORDER:
            x_i, y_i = WHEEL_POS[name]
            r_world = np.array(
                [cos_yaw * x_i - sin_yaw * y_i, sin_yaw * x_i + cos_yaw * y_i],
                dtype=np.float32,
            )
            v_wheel_des = v_des_world + omega_des * np.array([-r_world[1], r_world[0]], dtype=np.float32)
            v_des_norm = float(np.hypot(v_wheel_des[0], v_wheel_des[1]))
            if v_des_norm < 1e-6:
                continue
            steer = self.robot._steer[name]
            speed = self.robot._speed[name]
            v_wheel = speed * np.array([np.cos(yaw + steer), np.sin(yaw + steer)], dtype=np.float32)
            eff_scores.append(float(np.dot(v_wheel, v_wheel_des) / (v_des_norm * max(self.robot.v_wheel_max, 1e-6))))
        eff_reward = float(np.mean(eff_scores)) if eff_scores else 0.0

        # Inverse-kinematics penalty:
        # 1) Desired chassis twist (v_ref, omega_ref) -> desired wheel speed + steering angle.
        # 2) Penalize deviation between actual wheel state and IK target (within limits).
        v_body_des = np.array(
            [cos_yaw * bdx + sin_yaw * bdy, -sin_yaw * bdx + cos_yaw * bdy],
            dtype=np.float32,
        )
        inv_speed_err = 0.0
        inv_steer_err = 0.0
        for name in WHEEL_ORDER:
            x_i, y_i = WHEEL_POS[name]
            v_wheel_des = v_body_des + omega_des * np.array([-y_i, x_i], dtype=np.float32)
            v_des_norm = float(np.hypot(v_wheel_des[0], v_wheel_des[1]))
            v_des_norm = float(np.clip(v_des_norm, -self.robot.v_wheel_max, self.robot.v_wheel_max))
            steer_des = float(np.arctan2(v_wheel_des[1], v_wheel_des[0]))
            steer_des = float(np.clip(wrap_to_pi(steer_des), -STEER_LIMIT, STEER_LIMIT))
            steer_act = float(self.robot._steer[name])
            speed_act = float(self.robot._speed[name])
            inv_speed_err += float((speed_act - v_des_norm) ** 2) / max(self.robot.v_wheel_max, 1e-6) ** 2
            inv_steer_err += float(wrap_to_pi(steer_act - steer_des) ** 2)

        # Steer continuity / flip / limit penalties.
        steer_cont_err = 0.0
        steer_flip_err = 0.0
        steer_limit_err = 0.0
        for name in WHEEL_ORDER:
            steer_now = float(self.robot._steer[name])
            steer_prev = float(self.robot._steer_prev_step.get(name, steer_now))
            dtheta = wrap_to_pi(steer_now - steer_prev)
            steer_cont_err += float(dtheta ** 2)
            if abs(dtheta) > self._steer_flip_threshold:
                steer_flip_err += float((abs(dtheta) - self._steer_flip_threshold) ** 2)
            limit_excess = max(0.0, abs(steer_now) - (STEER_LIMIT - self._steer_limit_margin))
            steer_limit_err += float(limit_excess ** 2)

        # Slip penalty: lateral velocity at each wheel in body frame.
        cos_yaw = np.cos(yaw)
        sin_yaw = np.sin(yaw)
        ...
        for name in WHEEL_ORDER:
            ...
            slip_penalty += float(slip_norm ** 2)

        progress = 0.0
        if self._last_reward_time != self.context.ref_time:
            curr_dist = float(np.hypot(px - rx, py - ry))
            progress = (self._prev_dist_ref - curr_dist) / max(self.follow_dist, 1e-6)
            self._prev_dist_ref = curr_dist
            self._last_reward_time = self.context.ref_time
        ...
        w_waste = 0.1
        w_eff = 0.6
        if self._w_ik_warm_steps <= 0:
            w_inv_speed = self._w_ik_speed_end
            w_inv_steer = self._w_ik_steer_end
        else:
            inv_frac = min(1.0, self._debug_step / self._w_ik_warm_steps)
            w_inv_speed = self._w_ik_speed_start + (
                self._w_ik_speed_end - self._w_ik_speed_start
            ) * inv_frac
            w_inv_steer = self._w_ik_steer_start + (
                self._w_ik_steer_end - self._w_ik_steer_start
            ) * inv_frac
        w_prog = 1.5
        ...
        if self._w_steer_smooth_warm_steps <= 0:
            w_steer_smooth = self._w_steer_smooth_end
        else:
            frac_s = min(1.0, self._debug_step / self._w_steer_smooth_warm_steps)
            w_steer_smooth = self._w_steer_smooth_start + (
                self._w_steer_smooth_end - self._w_steer_smooth_start
            ) * frac_s
        if self._w_ik_warm_steps <= 0:
            w_steer_cont = self._w_steer_cont_end
        else:
            frac_c = min(1.0, self._debug_step / self._w_ik_warm_steps)
            w_steer_cont = self._w_steer_cont_start + (
                self._w_steer_cont_end - self._w_steer_cont_start
            ) * frac_c
        w_move = 1.0
        move_bonus = float(np.hypot(vx, vy) / max(self.v_max, 1e-6))
        to_target = np.array([rx - px, ry - py], dtype=np.float32)
        dist_target = float(np.hypot(to_target[0], to_target[1]))
        if dist_target > 1e-6:
            dir_x = to_target[0] / dist_target
            dir_y = to_target[1] / dist_target
        else:
            dir_x, dir_y = 0.0, 0.0
        move_align = float((vx * dir_x + vy * dir_y) / max(self.v_max, 1e-6))
        move_align = float(np.clip(move_align, -1.0, 1.0))
        w_align = 0.6
        lat_vel = float((-vx * dir_y + vy * dir_x) / max(self.v_max, 1e-6))
        lat_penalty = float(lat_vel ** 2)
        w_lat = 0.4
        wheel_align = 0.0
        for name in WHEEL_ORDER:
            steer = self.robot._steer[name]
            wheel_align += np.cos(steer) * dir_x + np.sin(steer) * dir_y
        wheel_align = float(wheel_align / len(WHEEL_ORDER))
        w_walign = 0.1
        wheel_proj = 0.0
        for name in WHEEL_ORDER:
            steer = self.robot._steer[name]
            speed = self.robot._speed[name] / max(self.robot.v_wheel_max, 1e-6)
            wheel_proj += speed * (np.cos(steer) * dir_x + np.sin(steer) * dir_y)
        wheel_proj = float(wheel_proj / len(WHEEL_ORDER))
        w_wproj = 1.5
        yaw_rate = float(self.robot.w_body)
        yaw_rate_norm = yaw_rate / max(self.w_max, 1e-6)
        ...
        reward = (
            ...
            -w_vel * vel_err
            -w_yaw * yaw_err
            -w_yaw_rate * (yaw_rate_norm ** 2)
            -w_yaw_accel * (yaw_accel ** 2)
            -w_yaw_ema * self._yaw_err_ema
            # Smooth control penalty (steer rate + wheel accel).
            -w_ctrl * ctrl_cost
            -w_dir * dir_penalty
            -w_waste * waste_penalty
            -w_slip * slip_penalty
            # IK tracking penalty (soft, warmed up).
            -w_inv_speed * inv_speed_err
            -w_inv_steer * inv_steer_err
            -w_steer_cont * steer_cont_err
            -self._w_steer_flip * steer_flip_err
            -self._w_steer_limit * steer_limit_err
            + w_eff * eff_reward
            + w_prog * progress
            + w_move * move_bonus
            + w_align * move_align
            + w_walign * wheel_align
            + w_wproj * wheel_proj
            - w_lat * lat_penalty
            - w_steer_smooth * float(np.sum((action[0::2] / max(self.robot.steer_rate_max, 1e-6)) ** 2))
        )
    ...
    def _get_ref_target(self) -> Tuple[float, float, float]:
        ...
        if self._last_ref_time != self.context.ref_time:
            ...
            yaw_ref_new = wrap_to_pi(self._yaw_ref_prev + beta * dyaw)
            self._yaw_ref_rate = wrap_to_pi(yaw_ref_new - self._yaw_ref_prev) / max(self.dt, 1e-6)
            self._yaw_ref_prev = yaw_ref_new
            self._last_ref_time = self.context.ref_time
